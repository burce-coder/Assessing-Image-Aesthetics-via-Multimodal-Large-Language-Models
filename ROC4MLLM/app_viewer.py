...
def get_args():
    ...
    # Use parse_known_args to avoid conflicts with Streamlit internal CLI arguments
...
# 5. Load Data with Caching for Performance
@st.cache_data
def load_data(path):
    try:
...
if search_query:
    filtered_data = [d for d in filtered_data if search_query.lower() in d.get('file_path', '').lower()]

# B. Threshold Logic Filter
if use_threshold:
    if filter_mode == ">= Threshold (Pass)":
        filtered_data = [d for d in filtered_data if d.get('score') is not None and d['score'] >= threshold_val]
    else:
        filtered_data = [d for d in filtered_data if d.get('score') is not None and d['score'] < threshold_val]

# 8. Sorting Logic
if sort_order == "High to Low":
    filtered_data = sorted(filtered_data, key=lambda x: x.get('score') if x.get('score') is not None else -1, reverse=True)
elif sort_order == "Low to High":
    filtered_data = sorted(filtered_data, key=lambda x: x.get('score') if x.get('score') is not None else 101)
...
display_batch = filtered_data[start_idx: start_idx + items_per_page]
        ...
        for j, item in enumerate(batch_row):
            with cols[j]:
                ...
                if score is not None:
                    ...
                    color = "green" if score >= current_threshold else "red"
                    st.markdown(f"### Score: :{color}[{score}]")
                else:
                    st.markdown("### Score: :gray[N/A]")

                # Display Image - UPDATED: use width="stretch" instead of use_container_width
                if os.path.exists(full_img_path):
                    st.image(full_img_path, width="stretch")
                else:
